...
MP.grammar = {
    ...
    BASE_URL: 'chrome://melampus-latin/content/grammar/',
     ...
     */
    onLoad: function() {
        var toc_doc = $("#mp-latin-grammar-toc").get(0).contentDocument;
        var content_browser = $("#mp-latin-grammar-content");
        ...
        document.getElementById("mp-latin-grammar-content")
            .addEventListener(
                "DOMContentLoaded",
                function() 
                {
                    $("a",this.contentDocument)
                        .click(MP.grammar.contentClickHandler);
                },
                true
                );
        ...
        if (typeof window.arguments != "undefined")
        {
            ...
            if (typeof params.update_args_callback == 'undefined')
            {
                params.update_args_callback =
                    function(a_args)
                        {
                            MP.grammar.set_start_href(a_args);
                        }
            }
        ...
        // src of the mp-latin-grammar-content iframe
        $("a",toc_doc).click(MP.grammar.tocClickHandler);
        ...
        $("h2.contents",toc_doc).click(MP.grammar.tocheadClickHandler);
     ...
     * of the mp-latin-grammar-content browser.
     */
    tocClickHandler: function(e)
    {
        var toc_doc = $("#mp-latin-grammar-toc").get(0).contentDocument;
        var href = $(this).attr("href");
        var href_target = MP.grammar.lookup_anchor(href.substring(1));
        if (href.indexOf('#') == 0 &&
            $("a[name='"+href+"']").length==0 &&
            typeof href_target != "undefined"
            )
        {
            $("#mp-latin-grammar-content").attr("src",
                MP.grammar.BASE_URL +
                href_target + href
            );
    ...
    contentClickHandler: function(e)
    {
        var href = $(this).attr("href");
        var href_target = 
            MP.grammar.lookup_anchor(href.substring(1));
        if (href.indexOf('#') == 0 &&
            $("a[name='"+href+"']").length==0 &&
            typeof href_target != "undefined"
            )
        {
            window.opener.MP.util.log("Resetting href to " + 
                MP.grammar.BASE_URL + 
                href_target + href);
            $(this).attr("href", 
                MP.grammar.BASE_URL + 
    ...
    {
        window.opener.MP.util.log("Reading latin grammar anchor map from file system");
        var io_service = 
        ...
        {
            window.opener.MP.util.log("Could not process grammar anchor_map: " + exception);
            window.opener.MP.util.log(buffer);
        }
    ...
    /**
     * get_anchor_map
     * Retrieves the anchor map from the browser's melampus object
    ...
    get_anchor_map: function() 
    {
        ...
        var bro = opener.MP.main.getCurrentBrowser();
        if (typeof bro.melampus.latin == "undefined" ||
            typeof bro.melampus.latin.grammar_anchor_map == "undefined")
        {
            
            // make sure the latin namespace in the melampus object
            // has been defined
            if (typeof bro.melampus.latin == "undefined")
            {
                bro.melampus.latin = {};            
            }
            bro.melampus.latin.grammar_anchor_map = this.load_anchor_map();
        }
    
        return bro.melampus.latin.grammar_anchor_map; 
    },
    ...
    set_start_href: function(params)
     {
        ...
        var start_href_target = MP.grammar.lookup_anchor(start_href);
